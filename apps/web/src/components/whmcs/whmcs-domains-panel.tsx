...
export function WhmcsDomainsPanel() {
  ...
  useEffect(() => {
    ...
    setError('');
    setHint('');
    let q = withWorkspaceQuery(API_PATHS.whmcs.domains, workspaceId);
    q += `${q.includes('?') ? '&' : '?'}limit=${LIMIT}&offset=${offset}`;
    apiFetch<{ rows: DomainRow[]; total: number }>(q, {}, getStoredToken()).then((res) => {
      setLoading(false);
      if (!res.success) {
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-slate-500">Domínios registados no WHMCS.</p>
        </div>
          ...
          <tbody>
            {rows.map((row) => (
              <tr key={row.id} className="border-t border-slate-100">
                ...
                <td className="px-3 py-2 font-medium text-slate-900">{row.domain || '—'}</td>
                <td className="px-3 py-2">{row.registrar || '—'}</td>
