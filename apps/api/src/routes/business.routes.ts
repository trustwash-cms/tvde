...
import { z } from 'zod';
import { canAssignRole, canManageUser, filterTvdeModules, type Role } from '@tvde/shared';
import { UserRole, Prisma } from '@tvde/database';
import { hashPassword, validatePasswordWithHibp } from '../lib/password';
...
export async function userRoutes(fastify: FastifyInstance) {
  ...
  fastify.get('/users', {
    ...
    const users = await fastify.db.user.findMany({
      where: q ? { AND: [baseWhere, textOr(q, ['email'])] } : baseWhere,
      select: {
        id: true,
        email: true,
        role: true,
  ...
  fastify.post('/users', {
    ...
    const pwdCheck = await validatePasswordWithHibp(body.password);
    if (!pwdCheck.valid) {
