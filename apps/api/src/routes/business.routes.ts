...
import { z } from 'zod';
import { canAssignRole, canManageUser, canToggleUserStatus, filterTvdeModules, formatWhatsappPhone, getRoleLabel, isDriverRole, isValidUsername, normalizeUsername, roleRequiresPhone, type Role } from '@tvde/shared';
...
import { EmailNotConfiguredError, sendUserWelcomeEmail } from '../services/email.service';
...
export async function userRoutes(fastify: FastifyInstance) {
  ...
  fastify.post('/users', {
    ...
    if (autoGenerated) {
      try {
        ...
        if (err instanceof EmailNotConfiguredError) {
          return reply.status(400).send({
            success: false,
            error: 'Utilizador criado mas SMTP não configurado — configure email e reenvie credenciais manualmente',
          });
        }
        throw err;
      }
    ...
    return reply.status(201).send({
      success: true,
      data: { ...user, credentialsSent: autoGenerated },
    });
  });
